...
export const getDayOrdersAmount = new Elysia()
  ...
  .get('/metrics/day-orders-amount', async ({ getCurrentUser }) => {
    ...
    const yestedayWithMonthAndYear = yesterday.format('YYYY-MM-DD')
    ...
    const yesterdayOrdersAmount = orderPerDay.find((orderPerDay) => {
      return orderPerDay.dayWithMonthAndYear === yestedayWithMonthAndYear
    })
    ...
    return {
      revenue: todayOrdersAmount?.amount,
